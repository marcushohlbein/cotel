...
class GoParser(Parser):
            ...
            if method_name:
                ...
                symbols.append(
                    Symbol(
                        ...
                        exported=self._is_exported(method_name),
        ...
        # Handle type declarations (structs, interfaces)
            ...
            for child in node.children:
                if child.type == "type_spec":
                    type_name = self._get_type_identifier(child)
                    if type_name:
                        symbol_id = str(uuid.uuid4())
                        kind = "interface"
    ...
    def _get_field_identifier(self, node):
        """Extract field_identifier from node (for method names)."""
        for child in node.children:
